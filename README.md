...
页面默认就展示**全部资产**，不再按分类拆分筛选（与「无限画布」模板一致）：网格库和「无限画布」展厅都直接铺满全部条目。
`category` 字段现在只作为卡片/详情里的**标签信息**展示，不再生成分类切换页签；分类筛选 UI（`#catList` / `#catChips` / `#canvasChips`）已在 `index.html` 中隐藏。
仍保留：顶部搜索（标题/分类/风格/模型/标签/提示词全文）与「模型」筛选。
